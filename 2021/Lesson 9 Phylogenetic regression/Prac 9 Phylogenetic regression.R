...
gls2 <- gls(x~1, d, correlation = corPagel(0.8 , phy = phy,fixed=T))
summary(gls2)
...
coef(summary(gls3))  #compare to lm1 -> identical!
coef(summary(lm1))
...
coef(summary(gls4))  #compare to lm1 -> identical!
...
coef(summary(gls5))  #compare to gls2 
coef(summary(gls2))  #-> identical
...
sd.e <- .15  #within sites variation
...
vcv.1 <- kronecker(1*diag(nrow=4), matrix(1, nrow=6, ncol=6))
vcv.1
#second impose between sites covariance with  maximum covariance along the main  diagonal (the covariance between a site and itself)
vcv <- vcv.1 + 0.001*diag(dim(d)[1])
vcv
# this is our covariance matrix describing the hierarchy in the data

#now turn this into a phylogeny 
phy.lm <- vcv2phylo(vcv, tolerance = 1e-4)
...
par(mfrow=c(1,2))
plot(phy.lm)
plot(d$x, 1:length(d$x), xlab="x", ylab="site")
...
coef(summary(z.lm)); coef(summary(z.lmm)); coef(summary(z.gls))
#importantly please note that the lmm is best, and the gls is second best, both far superior to the lm

#for those who want to think about this more deeply, please appreciate that the covariance matrices for the lmm and the gls are not identical
...
hist(z.lam$bootstrap[,3], main="lambda", xlab="")
...
z.0 <- lm(x ~ 1) #set up null model (i.e. lambda =0, alpha=50); draw bootstraps off this distribution, then estimate lambda and alpha to get the null distributions for these two parameters
summary(z.0)
nboot <- 500
...
meandata <- read.csv("/Users/kyletomlinson/Dropbox/Teaching/XTBG advanced stats 2022/Lectures/Lesson 9 Phylogenetic regression/exampledata/Savanna_seedlings.csv", header=TRUE, na.strings="*")
summary(meandata)
dim(meandata)
head(meandata)
...
tree1 <- read.nexus("/Users/kyletomlinson/Dropbox/Teaching/XTBG advanced stats 2022/Lectures/Lesson 9 Phylogenetic regression/exampledata/Savanna_seedling_nexus",tree.names = NULL)
tree1
...
#ignore the warning
z.lam <- phylolm(RMF ~ 1, data = meandata2,phy=combine$phy, model = "lambda")
summary(z.lam)  #same result as above

z.OU <- phylolm(RMF ~ 1,data = meandata2, phy=phy.min, model = "OUfixedRoot")
summary(z.OU) 
...
# x<- meandata2$RMF
x <- meandata2 %>% select(RMF)
x[1,]
phy <- phy.min
nperm <- 500
perm <- data.frame(lam=rep(0,nperm), alpha=0)
for(i in 1:nperm){
  x.perm <- x %>% slice(sample(1:nrow(x)))
  row.names(x.perm) <- row.names(meandata2)
  z.lam.perm <- phylolm(RMF ~ 1, data = x.perm, phy=phy, model = "lambda")
  z.OU.perm <- phylolm(RMF ~ 1, data = x.perm, phy=phy, model = "OUfixedRoot")
  perm$lam[i] <- z.lam.perm$optpar
  perm$alpha[i] <- z.OU.perm$optpar
}
...
#lambda
P.lam <- mean(perm$lam < z.lam.1$optpar)
hist(perm$lam, main=paste("P =", round(P.lam, digits=3)), xlab="")
lines(z.lam.1$optpar * c(1,1), c(0,nperm), col="red")
...
#a loop will help you to do this..
