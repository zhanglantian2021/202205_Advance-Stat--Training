...
x <- rTraitCont(phy, model = "BM", sigma = 1) #BM=Brownian motion
...
TreeCovar.0.0 <- vcv.phylo(phy.lam.0.0) #off diagonals all zero and diagnoals all 1 so its identity matrix
...
z.0 <- lm(x ~ 1) #set up null model (i.e. lamda =0 and alpha =50) draw bootstraps off this distributon, then estimate lamda and alpha to get th null distributions for these two parameters.
summary(z.0)
nboot <- 500
