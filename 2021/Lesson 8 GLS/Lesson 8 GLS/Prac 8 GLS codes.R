...
summary(mod2a) 
## look at the power estimate: roughly matches the simulated parameter (0.5)
...
treebase <- read.csv('Foresttype_BA.csv')
summary(treebase)
head(treebase,n=10)
...
plantdamage <- read.csv('plantdamage.csv', h=T)
summary(plantdamage)
head(plantdamage)
dim(plantdamage)

plantdamage$shadehouse<-as.factor(plantdamage$shadehouse)
plantdamage$sdl.no<-as.factor(plantdamage$sdl.no)
plantdamage$species<-as.factor(plantdamage$species)
plantdamage$light<-as.factor(plantdamage$light)
summary(plantdamage)
mod<-lme(growth~damage*light, random =~1|shadehouse, data=plantdamage)
summary(mod)



#model from wchat
mod.lme1<-lme(growth~damage+light,random=~1|shadehouse,weights=varIdent(form=~1|light), data=damage)

plot(mod.lme1, (abs(resid(., type='p')))^(1/2)~fitted(.)|light,type=c('smooth', 'p')) 
